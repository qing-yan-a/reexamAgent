"""
langmemory_postgres.py — 全数据库版（持久化，重启后数据不丢失）

- Checkpointer: PostgresSaver（线程内对话历史，PostgreSQL）
- Store: PostgresStore（跨线程记忆，PostgreSQL + 向量搜索）
...
# ============================================================
# Voyage Embedding（供 PostgresStore 向量搜索用）
# ============================================================
...
# Store：跨线程记忆（PostgreSQL，持久化，支持向量搜索）
_postgres_store = PostgresStore.from_conn_string(
    get_postgres_uri(),
    index={
        "dims": 1024,
        "embed": embed_texts,
...
store = _postgres_store.__enter__()
store.setup()

# Checkpointer：线程内对话历史（PostgreSQL，持久化）
